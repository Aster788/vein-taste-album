...
网站运行时：
  Mapbox 读取 restaurants.json 中的 lng/lat 字段渲染标注点
...
> 非中国城市（济州岛、吉隆坡、马六甲等）使用谷歌坐标，原生 WGS-84，无需转换。

### 1.5 城市列表（共13本书）

> 编号与书架横向排列顺序一致，以 `src/utils/citySlugs.js` 中 `CITY_SLUGS` 为准。

| 编号  | 城市中文 | 城市英文         | 国家   | 语言类型  | 路由 slug |
| --- | ---- | ------------ | ---- | ----- | ------- |
| 1   | 上海   | Shanghai     | 中国   | 中文城市  | `shanghai` |
| 2   | 南京   | Nanjing      | 中国   | 中文城市  | `nanjing` |
| 3   | 苏州   | Suzhou       | 中国   | 中文城市  | `suzhou` |
| 4   | 青岛   | Qingdao      | 中国   | 中文城市  | `qingdao` |
| 5   | 重庆   | Chongqing    | 中国   | 中文城市  | `chongqing` |
| 6   | 广州   | Guangzhou    | 中国   | 中文城市  | `guangzhou` |
| 7   | 济州岛  | Jeju         | 韩国   | 多语言城市 | `jeju` |
| 8   | 吉隆坡  | Kuala Lumpur | 马来西亚 | 多语言城市 | `kuala-lumpur` |
| 9   | 马六甲  | Melaka       | 马来西亚 | 多语言城市 | `melaka` |
| 10  | 福州   | Fuzhou       | 中国   | 中文城市  | `fuzhou` |
| 11  | 泉州   | Quanzhou     | 中国   | 中文城市  | `quanzhou` |
| 12  | 厦门   | Xiamen       | 中国   | 中文城市  | `xiamen` |
| 13  | 大连   | Dalian       | 中国   | 中文城市  | `dalian` |
...
- `cuisine_zh` / `cuisine_en`：菜系中文展示名 + 贴纸 slug（须与 `src/assets/stickers/cuisine/{cuisine_en}.svg` 文件名一致，如 `korean-cuisine`）。筛选、地图标签菜系文案用 `cuisine_zh`；贴纸与筛选键用 `cuisine_en`。详见 [docs/structure.md](structure.md) §菜系筛选贴纸
- `cuisine`：遗留列，`data:sync` 时与 `cuisine_zh` 同步；新填表请写 `cuisine_zh` / `cuisine_en`
...
- `store_slug`：同城多分店时**共用同一 slug**（见下节）；地图与菜品页共用此键，语义不同（地图按行打点，菜品按 slug 归组）
- `record_scope`：`branch` = 具体分店（满足下图「地图可见」条件时可上图）；`brand` = 品牌占位（无地图点）。多分店场景下各分店均为 `branch`，**不要**用 `brand` 表示分店
- `closed`：填 `yes`（大小写不敏感）表示已关闭；**不上地图**，菜品页仍展示
- `address` 为 `连锁店`（trim 后精确匹配）：连锁店占位地址，**不上地图**，菜品页仍展示

**地图可见**（`getMappableRestaurantsByCity`，菜品页不受此限制）：须同时满足 `record_scope=branch`、有效 `lng/lat`，且**不**满足以下任一排除条件：`record_scope=brand`、`closed=yes`、`address=连锁店`

#### 多分店（同一品牌多家门店）——通用规则

> **适用于所有店铺**：只要同城多条 `branch` 共用 `store_slug`，前端自动归组，**禁止**在组件里按店名/slug 写白名单或特殊分支。

同一城市、同一品牌的多条 `branch` 行使用**相同** `store_slug`，各自填写 `lng` / `lat` / `address` / 含分店后缀的 `name_zh`。

| 示例 | `store_slug` | 分店 `name_zh` |
|------|--------------|----------------|
| 南里山房 | `nlsf` | `南里山房(上海来福士广场店)`、`南里山房(上海荟聚店)` |
| 耶里夏丽 | `ylxl` | `耶里夏丽(田林东路店)`、`耶里夏丽·新疆菜·西域宴(徐汇旗舰店)` |

| 页面 | 行为 |
|------|------|
| 地图 | 每条**地图可见**的 `branch` 行独立打点（见上「地图可见」）；标签文案见下「地图标签店名」 |
| 菜品 | 左侧列表按 `(city_en, store_slug)` **合并为一项**，店名显示不含分店后缀的基础名及 `name_en` / `name_local`；右侧地址多行：`{分店标签}：{address}` |

**地图标签店名**（不随语言切换；字段优先级 `name_zh → name_en → name_local`；实现：`pickMapTagDisplayName` in [`src/utils/storeGroups.js`](../src/utils/storeGroups.js)）：

- **单店**（同城该 `store_slug` 仅一条 `branch` 行）：标签去掉末尾 `()` / `（）` 及括号内分店后缀，例如 `3号仓库·创意中国菜(五角场店)` → `3号仓库·创意中国菜`。
- **多分店**（同城同 `store_slug` 的 `branch` 行 ≥ 2）：各点标签保留**完整**店名，例如 `南里山房(上海来福士广场店)`、`南里山房(上海荟聚店)`。
- **地图右侧店铺详情**（点击标签后）：始终展示完整 `name_zh`（及 `name_en` / `name_local`），不受标签去括号影响。
- 空间不足时由 `formatMapTagLabel` 从标签文案末尾截断加 `…`；**禁止**只展示括号内分店后缀（如仅「打浦桥店」）。

**代码实现（唯一入口）**：[`src/utils/storeGroups.js`](../src/utils/storeGroups.js)。新增分店**只改 Excel**，不改 React。校验：`npm run audit:multi-branch`。

`dishes.json`：该组只维护一份 `store_slug`；`store_name_*` 写**基础店名**（如 `南里山房`、`耶里夏丽`），不写括号分店名。照片目录：`assets/photos/{city}/{store_slug}/` 一套。

### 5.2 菜品数据文件：dishes.json

独立文件，与 restaurants.json 通过 `store_slug`（同城一致时优先）及 `store_name_zh` / `store_name_en` / `store_name_local`（任一可匹配）关联，并携带 `store_slug` 作为图片目录定位键。
...
- 图片匹配规则（所有城市统一）：代码取图片 basename（去扩展名）后，按顺序匹配菜品 `dish_name_local` → `dish_name_en` → `dish_name_zh`；比对前需做 `trim` 与 Unicode 规范化（NFC）。**精确一致**时展示 `dishes.json` 菜名与价格/星级/备注；**basename 包含菜名但非完全一致**（如 `菜名-横截面`）时仅展示完整 basename，不挂菜品信息；`includes()` 前缀匹配仅用于排序分组（实现见 `src/utils/storePhotos.js`）
- 同店图片轮播顺序（四段）：① 匹配菜名（跨菜品按菜名拼音 `A-Z`；同菜内精确匹配图在前、前缀变体图紧跟其后）；② 匹配店名（`dishes.json` 的 `store_name_*`、去括号基础名、同 slug 分店 `restaurants.json` 的 `name_*`）；③ 其余非中文数字序号；④ 中文数字序号按数值升序
...
store_slug / record_scope /
name_zh / name_en / name_local /
cuisine_zh / cuisine_en / cuisine / address /
...
- 必填：`store_slug`、`record_scope`（`branch` / `brand`）
- 名称要求：`name_zh` / `name_en` / `name_local` 至少一列有值
- 菜系：`cuisine_zh`（中文展示）与 `cuisine_en`（贴纸 slug，与 `stickers/cuisine/{cuisine_en}.svg` 一致）成对填写；约定见 [docs/structure.md](structure.md) §菜系筛选贴纸、[src/data/README.md](../src/data/README.md)
- 其余列均可为空（非必填）
...
- `hours` 文本统一规则：
  ...
  - 非连续但同时段的多个日期段用 `、` 合并。
- `hours` 填表与展示规则：
  - 多段营业时间写在同一单元格，用英文 `|` 或全角 `｜` 分隔（两种均支持）；
  - Excel 中不换行；店铺详情页（笔记区 `NotePanel`）展示时按分隔符拆成多行显示。
- `phone` 填表与展示规则：
  - 多个号码写在同一单元格，用英文 `;` 或中文 `；` 分隔（两种分隔符均支持）；
  - Excel 中不换行；店铺详情页（笔记区 `NotePanel`）展示时去掉分隔符并按号码逐行换行显示。
- 重跑保护：
  - `name_zh`、`hours`、`cuisine_zh` / `cuisine_en` 在单元格已有非空值时不覆盖；
  - 高德等脚本首跑可自动填菜系时，写入 `cuisine_zh`（及建议同步填 `cuisine_en`）；`cuisine` 遗留列仅作 sync 兼容。
...
- 必填：`store_slug`
- 菜名要求：`dish_name_zh` / `dish_name_en` / `dish_name_local` 至少一列有值（全空行不入 `dishes.json`；店铺相册仍由 `photos` 目录驱动）
- 其余列均可为空（非必填）
...
- 店铺文件夹：使用 `store_slug`（来源于 `restaurants.xlsx` / `restaurants.json`）
  - 格式：`[a-z0-9-]+`
  - 唯一性：在 **dishes / photos** 维度，`(city_en, store_slug)` 对应一套菜品与相册；`restaurants.json` 中允许多条 `branch` 行共享同一 `(city_en, store_slug)`（多分店）
  - 语义：仅作资源路径键，不要求等于 `name_en` / `name_local` / `name_zh`
- 图片文件名：允许使用任一已知菜名语言版本（`dish_name_local` / `dish_name_en` / `dish_name_zh`），推荐优先本国语言
- 代码匹配与展示：详见 [project_rules.md](project_rules.md) §五补充规则；实现见 `src/utils/storePhotos.js`、`src/components/DishInfo.jsx`
  - basename 与菜名**精确一致** → 展示 `dishes.json` 菜名及价格/星级/备注
  - basename **包含**菜名但非完全一致（如 `菜名-横截面.jpg`）→ 仅展示完整 basename，不挂菜品信息；排序紧跟该菜精确匹配图之后
  - `includes()` 前缀匹配仅用于排序分组，不决定展示文案
- 轮播顺序：菜名图（跨菜品拼音序；同菜内精确先于前缀变体）→ 店名图 → 其它非中文序号图 → 中文序号图（数值序）
...
> **MVP 已全部完成，当前处于打磨/扩展阶段。** 日常维护见 [data-workflow.md](data-workflow.md) §0。下列 Phase 记录保留作历史参考，新任务不必再按 Phase 边界拆分。
>
...
- 下载菜系筛选贴纸 SVG，放入 `src/assets/stickers/cuisine/`，文件名 = `{cuisine_en}.svg`（如 `japanese-cuisine.svg`、`chuan-cuisine.svg`）；slug 与填表约定见 [docs/structure.md](structure.md) §菜系筛选贴纸
...
- 配置全局 CSS 变量（13 套城市配色 token、字体变量、纸质纹理）
...
- 城市详情顶栏：返回书架按钮（书架视口书脊条缩略图，→ `/`）+ 语言切换（中国城 `EN/CN`；非中国城按 §2.5）；书架页不显示语言切换
...
- 标注点连线引出店铺名标签（`pickMapTagDisplayName`：单店去括号、多分店完整名；详情区完整 `name_zh`；不随语言切换；见 §5.1）
- 右侧书脊菜系便利贴标签，点击筛选高亮/变灰
- 右侧笔记信息区：默认引导文字状态
- 点击标注点：笔记区展示店铺信息（店名按作者数据展示且不随语言切换；店名右侧地图跳转图标；菜系展示 `cuisine_zh` 且不随语言切换；综合评分、人均、营业时间、雷达图等字段标签随语言策略展示）
...
- 图片右侧菜品信息区（菜名层 + 评价层；basename 规则见 §5.5 / [project_rules.md](project_rules.md) §五；语言切换见 [prd-i18n-locale.md](prd-i18n-locale.md)）
- 中国城：basename 精确匹配时菜名随 EN/CN 在 `dish_name_zh` ↔ `dish_name_en` 切换（`allowMachineTranslate: false`）；前缀变体图仅显示 basename
- 非中国城市：精确匹配时固定多行 `dish_name_zh` → `dish_name_en` → `dish_name_local`；前缀变体图仅显示 basename
- 未精确匹配任何菜品时：图片仍展示；中文数字序号 basename 仅显示图片，其它 basename 显示为图片名称（不含扩展名）
...
- 填入除试点城外其余城市数据（GeoJSON + restaurants + dishes + 图片）
- 整体贴纸位置微调
...
> **日常维护**见 [data-workflow.md](data-workflow.md) §0；Phase 分阶段计划已归档（§六）。

1. 小任务可直接说明范围；大型改动仍建议分步验收
2. 喂给 Cursor：相关专题文档（`prd-ui-spec.md`、`prd-i18n-locale.md`、`project_rules.md` 等）+ `agent_rules.md`，不必每次全文喂入 `prd.md`
3. 数据与照片变更走 [data-workflow.md](data-workflow.md)；部署见 [deploy-vercel.md](deploy-vercel.md)
4. 改完在浏览器验证；相册 CDN 验收见 `data-workflow.md` §9.5
