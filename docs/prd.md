...
- 同店图片轮播顺序（四段）：① 匹配菜名；② 匹配店名（`dishes.json` 的 `store_name_*`、去括号基础名、同 slug 分店 `restaurants.json` 的 `name_*`）；③ 其余非中文数字序号；④ 中文数字序号按数值升序
...
- 代码匹配：basename 先匹配菜名（`dish_name_*`）；其次匹配店名（`dishes.json` 的 `store_name_*` 全文与去括号基础名、同 slug 的 `restaurants.json` `name_*`）
- 轮播顺序：菜名图 → 店名图 → 其它非中文序号图（英文起头字母序、中文起头拼音序）→ 中文序号图（数值序）
