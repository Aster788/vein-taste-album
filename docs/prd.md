...
- 书脊朝外，用户从书脊看这排书
- 书脊文字：竖向两行叠放，**上行中文、下行英文**，格式为「国家·城市」，与 §2.5「书脊与封面」一致，10 城统一（中国城与非中国城同一规则）。示意（上→下）：
  ```
  中国·大连     ← 上行：中文（霞鹜文楷）
...
- **店铺名称展示规则**：
  - **中国城市**：店铺名随语言切换在 `name_zh` / `name_en` 间切换展示（若某一侧为空则回退另一侧）
  - **非中国城市**：店铺名**不参与**语言切换；在列表中按固定顺序输出非空行：`name_zh` → `name_en` → `name_local`（每行一个；有多少语言就显示多少行）
