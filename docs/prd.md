...
- 同店图片轮播顺序（四段）：① 匹配菜名（同菜内精确匹配图在前、前缀变体图紧跟其后）；② 匹配店名（`dishes.json` 的 `store_name_*`、去括号基础名、同 slug 分店 `restaurants.json` 的 `name_*`）；③ 其余非中文数字序号；④ 中文数字序号按数值升序
...
- 轮播顺序：菜名图（同菜内精确先于前缀变体）→ 店名图 → 其它非中文序号图 → 中文序号图（数值序）
