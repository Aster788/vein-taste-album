...
- **部署目标**：Vercel 静态托管；店铺照片在 **Cloudflare R2**（`VITE_PHOTOS_BASE_URL`），本机用 `npm run photos:sync-r2` **默认增量**同步（见 [data-workflow.md](data-workflow.md) §9.2）；全量仅 `--full` 例外场景
...
- **目前共 13 个城市**：中国 10 城（上海、南京、苏州、青岛、重庆、广州、福州、泉州、厦门、大连）+ 济州岛 + 吉隆坡 + 马六甲
